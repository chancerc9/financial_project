...
import model_portfolio_process as bench
...
def create_indexData_table(solution_df, given_date, ftse_data: pd.DataFrame, asset_type='public', curMonthBs=False):
    benchmarking_solution = solution_df.copy()
    benchmarking_solution.rename(columns={5: 6, 4: 5, 3: 4, 2: 3, 1: 2, 0: 1}, inplace=True)
    benchmarking_solution['portfolio'] = benchmarking_solution['portfolio'].replace({'Total': 'TOTAL',
                                                                                     'np': 'NONPAR',
    ...
    # over_35y = (100 - ftse_data.loc[ftse_data['TermPt'] >= 35.25]['marketweight_noREITs'].sum()) / 100 # gets all weights that are strictly less than 35.25y
...
def main_test():
    ...
    public_solution = bench.optimization(GivenDate, asset_type='public', swap=args.swap,
